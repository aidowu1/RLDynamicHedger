...
def APL_process(S, p, holding, K, notional, kappa):
    ...
    #APL \
    term_1 = -(p - p_lagged)
    term_2 = holding_lagged*(S-S_lagged)
    term_3 = -kappa* np.abs(S*(holding - holding_lagged))
    APL = term_1 + term_2 + term_3

    term_4 = -(np.maximum((S[:,-1] - K), 0)*notional - p_lagged[:,-1])
    term_5 = holding_lagged[:,-1]*(S[:,-1] - S_lagged[:,-1])
    term_6 = -kappa* np.abs(S[:,-1]*(holding[:,-1] - holding_lagged[:,-1]))

    APL[:, -1] = term_4 + term_5 + term_6
